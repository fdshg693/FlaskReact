...
def main():
    ...
    # Model selection
    parser.add_argument(
        "--model", "-m", type=str, help="Path to model checkpoint (.pth file)"
    )

    # Input specification
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--image", "-i", type=str, help="Path to single image file")
    group.add_argument(
        "--directory", "-d", type=str, help="Path to directory containing images"
    )

    # Options
    parser.add_argument(
        "--output", "-o", type=str, help="Output file to save results (JSON format)"
    )
    parser.add_argument(
        "--visualize",
        "-v",
    ...
    try:
        ...
        evaluator = ModelEvaluator(
            checkpoint_path=args.model, device=args.device, img_size=args.img_size
        )
        ...
        if args.image:
            ...
            if args.visualize:
                ...
                evaluator.visualize_prediction(
                    args.image, save_path=vis_path, show_image=True
                )
