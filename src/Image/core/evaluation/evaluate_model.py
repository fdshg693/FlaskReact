...
class EvaluationGUI:
    ...
    def setup_ui(self):
        ...
        self.checkpoint_combo = ttk.Combobox(
            model_frame, textvariable=self.checkpoint_var, state="readonly", width=50
        )
        ...
        ttk.Button(model_frame, text="Browse...", command=self.browse_checkpoint).grid(
            row=0, column=3, padx=(5, 0)
        )

        # Status
        self.status_var = tk.StringVar(value="No model loaded")
        ttk.Label(model_frame, textvariable=self.status_var, foreground="red").grid(
        ...
        ttk.Button(image_frame, text="Select Image", command=self.select_image).grid(
            row=0, column=0
        )

        self.image_path_var = tk.StringVar(value="No image selected")
        ttk.Label(image_frame, textvariable=self.image_path_var).grid(
            row=0, column=1, sticky=(tk.W, tk.E), padx=(10, 0)
        )

        ttk.Button(image_frame, text="Evaluate", command=self.evaluate_image).grid(
            row=0, column=2, padx=(5, 0)
        )
        ...
        pred_scrollbar = ttk.Scrollbar(
            pred_text_frame, orient=tk.VERTICAL, command=self.pred_text.yview
        )
        ...
        ttk.Button(
            export_frame, text="Export Results", command=self.export_results
        ).pack(side=tk.LEFT)

        ttk.Button(
            export_frame, text="Batch Evaluate Directory", command=self.batch_evaluate
    ...
    def load_model(self):
        ...
        try:
            ...
            self.status_var.set(
                f"Model loaded successfully ({self.evaluator.num_classes} classes)"
            )
    ...
    def evaluate_image(self):
        ...
        try:
            ...
            def run_evaluation():
                self.current_result = self.evaluator.predict_single_image(
    ...
    def batch_evaluate(self):
        ...
        def run_batch_evaluation():
            try:
                # Update status
                self.root.after(
                    0, lambda: self.status_var.set("Running batch evaluation...")
                )
                ...
                # Show results
                self.root.after(
                    0, lambda: self.show_batch_results(summary, output_file)
                )
            finally:
                self.root.after(
                    0,
