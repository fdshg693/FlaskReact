...
## Prerequisites
> **Important**: To view problems for a specific file, you must first open that file in the editor by making a trivial modification (e.g., adding/removing a newline, inserting a space). This triggers the language server to analyze the file and populate diagnostics. Without this step, `#tool:read/problems` may return incomplete results.

## Task
Systematically identify, analyze, and resolve all Python type errors by following the workflow below.

## Workflow
1. **Open Target Files** — For each file to analyze, make a minimal edit (e.g., append newline) to trigger diagnostic loading.
2. **Gather Diagnostics** — Call `#tool:read/problems` to collect all current type errors with file paths, line numbers, and error codes.
3. **Categorize & Prioritize** — Group errors by type (e.g., incompatible types, missing annotations, generic violations) and identify dependencies between fixes.
4. **Create Fix Plan** — Use `#tool:todo` to generate a structured remediation plan with:
