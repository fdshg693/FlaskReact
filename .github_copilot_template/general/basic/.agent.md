...
custom_inputs:
  - name: context_path
    type: string
    description: AIにプロジェクトの全体像を把握させるためのコンテキストファイルパス。
    default: README.md
outputs:
  - name: default
    context_path: .ai/knowledge/entry/general.md
---
<context>
...
<workflow>
  <step number="1" name="Information Gathering">
    ...
    If yes, use #tool:web/fetch to retrieve relevant data.
  </step>
  ...
  <step number="3" name="Task Planning">
    Use #tool:todo to create a structured list of todos for the request.
  </step>
  
  <step number="4" name="Task Delegation">
    For each todo item, use #tool:agent/runSubagent to delegate the task to an appropriate subagent.
    Wait for completion before proceeding to the next task.
  </step>
  
  <step number="5" name="Verification">
    After all tasks are complete, use #tool:read/problems to check for any issues or errors.
  </step>
</workflow>
...
<example_output>
  ...
  <step_4>
    #tool:agent/runSubagent task="Create auth middleware" context="${project_root}/src/middleware"
    #tool:agent/runSubagent task="Add login endpoint" context="${project_root}/src/routes"
    #tool:agent/runSubagent task="Add token validation" context="${project_root}/src/utils"
  </step_4>
