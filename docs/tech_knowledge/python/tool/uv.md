...
### スクリプトの実行
- 実行する際に仮想環境が自動でアクティブになります。そのため、事前の仮想環境のアクティブ化は不要です。
```bash
uv run `*.py`
```
...
### ツールをパスに追加
```bash
echo 'export PATH="/Users/author/.local/bin:$PATH"' >> ~/.zshrc
source ~/.zshrc
```
パスにない場合のエラー内容
- warning: `/Users/author/.local/bin` is not on your PATH. To use installed tools, run `export PATH="/Users/author/.local/bin:$PATH"` or `uv tool update-shell`.

## 自作パッケージのエディタブルインストール
```bash
# ローカルパッケージを依存関係として追加（自動的にエディタブル）
uv add --editable /path/to/local-package

# 同一ワークスペース内のパッケージなら
uv add --editable ./packages/my-lib
```

- TOMLファイルには、以下のように表示される
```toml
[tool.uv.sources]
